...
if (argc == 2) {
   parseConfigFile(argv[1], CREATE_STRUCT_PATHS, &h_page, &fld_nm_lst, config, &vNTL, QUIET);	// create the structure paths for parsing/populating verbose
   ...
   freeObjByNameList((void **)myNamespaceList);
   printf("\n");
   ...
   freeObjByNameList((void **)myRepoList);

   freeConfigStructContent(config, &vNTL, QUIET);								// free the config structure
   }
else
   printf("Usage: ./example-4 <XML config file>\n");
}
